...
long long int str_to_signed_int(char* string)
{
	#define ZERO 48
...
short verify_signed_double(char* string, double* converted_double)
{
	#define MAX_BUFFER_SIZE 600
	#define MAX_VALID_DOUBLE .0000000000000000
	#define MIN_VALID_DOUBLE -.0000000000000000
	
	double MAX_DOUBLE = MAX_VALID_DOUBLE;
	double MIN_DOUBLE = MIN_VALID_DOUBLE;
	...
	if(!is_digit(string[count]) && string[count] == '-'){ is_negative = 1; ++count;}
	else if(!is_digit(string[count]) && string[count] == '.'){ ++decimal_count; ++count; }
	else if(!is_digit(string[count])){ return (FALSE); }
	...
	if(is_negative){ supplied_double *= -1; }
	if(supplied_double <= MAX_DOUBLE && supplied_double >= MIN_DOUBLE){ *converted_double = supplied_double; return (TRUE); }
	else {printf("ERROR: Number too large or too small for storage.\n"); }
	return (FALSE);
	
	#undef MAX_BUFFER_SIZE
	#undef MAX_VALID_DOUBLE
	#undef MIN_VALID_DOUBLE	
}
...
short verify_double(char* string, double* converted_double)
{
	...
	while(is_space(string[count])) { ++count; }
	if(count == length) { return (FALSE); }
	if(!is_digit(string[count]) && string[count] == '.'){ ++decimal_count; ++count; }
	else if(!is_digit(string[count])) { return (FALSE); }
...
short verify_signed_integer(char* string, long long int* converted_integer)
{
	#define MAX_BUFFER_SIZE 488
	#define MAX_VALID_INTEGER 72036854775807
	#define MIN_VALID_INTEGER -72036854775800
	long long int MAX_INTEGER = MAX_VALID_INTEGER;
	long long int MIN_INTEGER = MIN_VALID_INTEGER;
	...
	char string_buffer[MAX_BUFFER_SIZE];
	size_t buffer_count = 0;
	long long int supplied_integer = 0;
	...
	string_buffer[buffer_count] = '\0';
	supplied_integer = str_to_signed_int(string_buffer);
	if(is_negative) { supplied_integer *= -1; }
	if(supplied_integer <= MAX_INTEGER && supplied_integer >= MIN_INTEGER) { *converted_integer = supplied_integer; return(TRUE); }
	else { printf("ERROR: Number too large or too small for storage.\n"); }
	return(FALSE);

	#undef MAX_BUFFER_SIZE
	#undef MAX_VALID_INTEGER
	#undef MIN_VALID_INTEGER
}
...
long long int safely_get_signed_int(char* prompt)
{
	char* input;
	long long int i_input;
	...
	do
	{
		...
		reloop = 1;
	}while(!verify_signed_integer(input, &i_input));
...
double safely_get_signed_double(char* prompt)
{
	char* input;
	double d_input;
	...
	do
	{
		...
		input = safely_get_input();
		reloop = 1;
	}while(!verify_signed_double(input, &d_input));
...
void new_int_node(Int_Node** list_head, long long int integer)
{
	// This method adds a new node to the beginning of the linked list
	// for struct types Int_Node
	char* memory_error_message = "MEMORY ALLOCATION ERROR! STOPPING EXECUTION!";
	...
	if(*list_head != NULL)
	{
		temp->next_link = *list_head;
	}
	*list_head = temp;
}

void print_int_list(Int_Node* list_head)
{
...
void free_int_list(Int_Node* node)
{
	Int_Node* temp;
	while(node != NULL)
	{
...
void new_double_node(Double_Node** list_head, double double_value)
{
	// This method adds a new node to the beginning of the linked list
	// for struct types Double_Node
	char* memory_error_message = "MEMORY ALLOCATION ERROR! STOPPING EXECUTION!";
...
void print_double_list(Double_Node* list_head)
{
	while(list_head != NULL)
	{
		printf("Double: %lf\n", list_head->double_value);
		list_head = list_head->next_link;
	}
}

void free_double_list(Double_Node* node)
{
	Double_Node* temp;
	while(node != NULL)
	{
