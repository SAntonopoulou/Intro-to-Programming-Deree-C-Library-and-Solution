...
size_t strip_suffix(char* string, char* new_string, char* suffix);

int main()
{
	char string[] = "Testsuffix";
	char suffix[] = "suffix"; 
	char string_without_suffix[(get_string_length(string) - get_string_length(suffix)) + 1];
	strip_suffix(string, string_without_suffix, suffix);
	printf("String with suffix: %s\n", string);
	printf("String without suffix: %s\n", string_without_suffix);	
}

size_t strip_suffix(char* string, char* new_string, char* suffix)
{
	// This function strips the suffix from a given string. It is extremelly similar to simply using
	// the function create_substring except that it checks that the string has the suffix to be stripped
	// in the first place. Simply using create_substring would work as well, but would not check that
	// the suffix is present.
	//
	// This function will return 0 if there has been an attempt to create the substring, and 1 if it did
	// not find the substring and did not attempt to create the substring.  
	if(!has_suffix(string, suffix))
	{
		size_t length_without_substring = ((get_string_length(string) - get_string_length(suffix)) - 1);
		create_substring(string, new_string, length_without_substring, 1);
		return 0;
	}
	return 1;
}
...
void create_substring(char *string, char* new_string, size_t number_of_characters, size_t direction)
{
	...
	//
	// This function returns 0 on successful creation of the substring and 1 if there are errors.
	size_t length = get_string_length(string);
	if(direction)
	{
		// forward
